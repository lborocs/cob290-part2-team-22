...
function EmpTodoList({ userId }) {
    ...
    const fetchTodos = async () => {
        try {
            const response = await fetch(`http://35.214.101.36/EmpToDoList.php?user_id=${userId}`);
    ...
    const handleSubmit = async () => {
        ...
        try {
            await fetch('http://35.214.101.36/EmpToDoList.php', {
                method,
    ...
    return (
        ...
        <Row className="justify-content-md-center">
            <Col md={8}>
                ...
                {showCharts ? (
                    <EmpTodoProgressCharts userId={userId} />
                ) : (
                  <>
